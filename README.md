...
Bitcoin Classic (BTCC) **一体化 macOS 客户端**：助记词钱包、转账、链上历史、OTC 行情、矿池算力查询和 Apple Silicon GPU 挖矿，全部集成在一个 SwiftUI 应用里。
...
| 模块 | 说明 |
|------|------|
| **钱包** | BIP39 创建/导入助记词、cc1 地址、余额查询、BTCC 转账 |
...
2. 创建后请务必备份 12 词助记词（保存在 `~/Library/Application Support/BTCCWallet/wallet.json`，不访问系统钥匙串）
