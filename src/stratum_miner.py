...
def submit_nonce_hex(nonce: int) -> str:
    ...
    Note: the block header itself stores the nonce in little-endian, but the
    Stratum protocol does NOT submit the LE header bytes — it submits the
    BE-formatted hex of the integer value, the same way mining.notify sends
    ntime/nbits. Sending the LE-bytes hex makes the pool reconstruct a
    different header, compute a different hash, and silently reject every
    share (the pool dashboard then shows "no shares received")."""
    return "%08x" % (nonce & 0xFFFFFFFF)
...
# ---------------------------------------------------------------------------
#
# Some networks (residential ISPs in mainland China being the obvious
# example) silently drop the application-layer traffic of overseas mining
# pools after the TCP three-way handshake completes — `nc -vz host port`
# succeeds but every subsequent JSON-RPC line is black-holed with no RST.
# When that happens the only reliable workaround is to tunnel the TCP
# connection through a working proxy.
#
# We support two industry-standard tunneling protocols, both implemented
# with Python's stdlib only:
#
#   - SOCKS5 (RFC 1928 + RFC 1929 username/password)  scheme: socks5 / socks5h
#   - HTTP CONNECT (RFC 7231 §4.3.6, RFC 7617 Basic)  scheme: http / https
#
...
#   ALL_PROXY=socks5://localhost:7890 ./scripts/start_stratum.sh ...
#
# The miner sends DNS resolution to the proxy itself (SOCKS5 ATYP=domain,
# HTTP CONNECT host:port) so a working proxy is enough — we don't also
# need DNS to the pool to resolve correctly from the local network.
...
def parse_proxy_url(url: str) -> ProxyConfig:
    """Parse a proxy URL like socks5://user:pass@host:1080 or http://host:8080."""
    if "://" not in url:
        url = "socks5://" + url
    parsed = urlparse(url)
    scheme = (parsed.scheme or "socks5").lower()
    # socks5h is the curl convention for "remote DNS"; we always do remote DNS,
    # so just normalize. https:// CONNECT is conceptually identical to http://
    # CONNECT on the wire (both speak plain HTTP/1.1 to the proxy itself).
    if scheme == "socks5h":
        scheme = "socks5"
    if scheme == "https":
        scheme = "http"
    if scheme not in ("socks5", "http"):
        raise ValueError(
            f"unsupported proxy scheme {parsed.scheme!r} (need socks5 or http)"
        )
...
def _recv_exact(sock: socket.socket, n: int) -> bytes:
    """recv exactly n bytes or raise. Used for fixed-length proxy framing."""
    buf = bytearray()
    while len(buf) < n:
        chunk = sock.recv(n - len(buf))
        if not chunk:
            raise OSError(f"proxy: unexpected EOF (got {len(buf)}/{n} bytes)")
        buf.extend(chunk)
    return bytes(buf)
...
def _socks5_handshake(sock: socket.socket, target_host: str, target_port: int,
    ...
    """Run RFC 1928 / 1929 client handshake on an already-connected socket."""
    methods = bytearray([0x00])  # NO AUTHENTICATION REQUIRED
    ...
    if rep[0] != 0x05:
        raise OSError(f"SOCKS5: bad version 0x{rep[0]:02x} in method reply")
    method = rep[1]
    if method == 0xFF:
        raise OSError("SOCKS5: proxy refused all offered auth methods")
    if method == 0x02:
        u = (username or "").encode("utf-8")
        p = (password or "").encode("utf-8")
        if not (1 <= len(u) <= 255) or not (0 <= len(p) <= 255):
            raise OSError("SOCKS5: username/password length out of range")
        sock.sendall(bytes([0x01, len(u)]) + u + bytes([len(p)]) + p)
        ar = _recv_exact(sock, 2)
        if ar[0] != 0x01 or ar[1] != 0x00:
            raise OSError(f"SOCKS5: auth failed (status=0x{ar[1]:02x})")
    elif method != 0x00:
        raise OSError(f"SOCKS5: proxy chose unsupported method 0x{method:02x}")

    # CONNECT request, ATYP=DOMAINNAME so the proxy resolves DNS for us.
    host_bytes = target_host.encode("idna") if any(
        ord(c) > 127 for c in target_host
    ) else target_host.encode("ascii")
    if not 1 <= len(host_bytes) <= 255:
        raise OSError(f"SOCKS5: target host length {len(host_bytes)} out of range")
    req = (
    ...
    rh = _recv_exact(sock, 4)
    if rh[0] != 0x05:
        raise OSError(f"SOCKS5: bad version 0x{rh[0]:02x} in connect reply")
    if rh[1] != 0x00:
        msg = _SOCKS5_REPLY_CODES.get(rh[1], "unknown")
        raise OSError(f"SOCKS5: connect failed (0x{rh[1]:02x} {msg})")
    atyp = rh[3]
    if atyp == 0x01:    # IPv4
        ...
        raise OSError(f"SOCKS5: unknown ATYP 0x{atyp:02x} in reply")


def _http_connect_handshake(sock: socket.socket, target_host: str, target_port: int,
                            username: Optional[str], password: Optional[str]) -> None:
    """Open an HTTP/1.1 CONNECT tunnel on an already-connected socket."""
    headers = [
    ...
    while b"\r\n\r\n" not in buf:
        ...
        if not chunk:
            raise OSError("HTTP proxy closed connection during CONNECT")
        buf.extend(chunk)
        if len(buf) > 65536:
            raise OSError("HTTP proxy CONNECT response header too large")
    head, _, leftover = buf.partition(b"\r\n\r\n")
    status_line = head.split(b"\r\n", 1)[0].decode("ascii", errors="replace")
    parts = status_line.split(" ", 2)
    if len(parts) < 2 or not parts[1].startswith("2"):
        raise OSError(f"HTTP proxy refused CONNECT: {status_line!r}")
    if leftover:
        # A correctly-behaved CONNECT proxy must not send data before the
        # client does. If it did, we'd have to splice that into our recv
        # buffer — fail loudly instead so the bug is visible.
        raise OSError(
            f"HTTP proxy returned unexpected payload before tunnel data "
            f"({len(leftover)} bytes)"
        )


def _connect_via_proxy(proxy: ProxyConfig, target_host: str, target_port: int,
                       *, timeout: float, prefer_ipv6: bool = False) -> socket.socket:
    """Open a TCP tunnel to (target_host, target_port) through `proxy`."""
    sock = _connect_socket(
        proxy.host, proxy.port,
        timeout=timeout, prefer_ipv6=prefer_ipv6,
    )
    try:
        sock.settimeout(timeout)
        if proxy.scheme == "socks5":
            _socks5_handshake(sock, target_host, target_port,
                              proxy.username, proxy.password)
        elif proxy.scheme == "http":
            _http_connect_handshake(sock, target_host, target_port,
                                    proxy.username, proxy.password)
        else:
            raise OSError(f"unsupported proxy scheme {proxy.scheme!r}")
    except Exception:
        try: sock.close()
        except Exception: pass
        raise
    print(
        f"[stratum] proxy tunnel ready ({proxy.scheme}) -> "
...
def resolve_proxy_from_args(args: argparse.Namespace) -> Optional[ProxyConfig]:
    """Resolve --proxy / ALL_PROXY / HTTPS_PROXY / HTTP_PROXY (in that order)."""
    candidates = [
    ...
    for source, val in candidates:
        if val:
            try:
                cfg = parse_proxy_url(val)
            except ValueError as e:
                print(f"[stratum] ignoring bad proxy from {source}: {e}", flush=True)
                continue
            return cfg
    return None
...
class StratumClient:
    ...
    def connect(self) -> None:
        if self.proxy is not None:
            self.sock = _connect_via_proxy(
                self.proxy, self.host, self.port,
    ...
    def call(self, method: str, params: list, timeout: float = 15.0) -> Any:
        ...
        try:
            ...
            try:
                ...
                raise TimeoutError(
                    f"no response to {method!r} within {timeout:g}s "
                    f"(connection dropped or pool silent)"
                ) from None
        finally:
            with self._lock:
...
def mine_session(args: argparse.Namespace, host: str, port: int,
                 *, helper: Optional["MetalGpuHelper"] = None,
                 proxy: Optional[ProxyConfig] = None) -> str:
    ...
    if proxy is not None:
        via = f" via {proxy.scheme}://{proxy.host}:{proxy.port}"
    print(f"[stratum] connecting to {host}:{port}{via} as {args.user!r} ...", flush=True)
    ...
    while client.connected:
        ...
        # Both ntime and nonce go on the wire as 8 hex chars representing the
        # uint32 value in big-endian order (the Stratum v1 convention used by
        # cgminer / ckpool / NOMP / Slush). NOT the LE bytes of the header.
...
def mine(args: argparse.Namespace) -> None:
    ...
    proxy_cfg = resolve_proxy_from_args(args)
    if proxy_cfg is not None:
        creds = "" if not (proxy_cfg.username and proxy_cfg.password) else " (auth)"
        print(
            f"[stratum] using {proxy_cfg.scheme} proxy "
            f"{proxy_cfg.host}:{proxy_cfg.port}{creds}",
            flush=True,
        )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--proxy", default=None,
                   help="Tunnel the stratum TCP connection through an outbound "
                        "proxy. Useful when a residential ISP silently drops "
                        "the application traffic to overseas mining pools "
                        "(connect succeeds but the pool sees no requests). "
                        "Examples: socks5://127.0.0.1:1080, "
                        "socks5://user:pass@host:1080, http://host:8080. "
                        "Falls back to ALL_PROXY / HTTPS_PROXY / HTTP_PROXY "
